...
def visualize_signal(signal, fs, zrc_arr=None, pr_arr=None, zrc_slope_arr=None, pr_slope_arr=None, ac_s=None, yin_diff=None, cepstrum=None, window_size=-1, window_start=0):
    ...
    if zrc_arr is None and pr_arr is None and zrc_slope_arr is None and pr_slope_arr is None and ac_s is None and yin_diff is None and cepstrum is None:
        ...
        if window_size == -1:
            ...
            plt.figure(figsize=(10,3))
            plt.subplot(1, 2, 1)
            plt.plot(time[window_start:window_start+window_size],signal[window_start:window_start+window_size])
            plt.xlabel('Time (s)')
            plt.ylabel('Amplitude')
            plt.title('Signal')
            plt.subplot(1, 2, 2)
            plt.plot(time[window_start:window_start+window_size],signal_gradient[window_start:window_start+window_size])
            plt.xlabel('Time (s)')
            plt.ylabel('Amplitude')
            plt.title('Signal Gradient')
            plt.show()
    ...
    if ac_s is not None:
        if window_size == -1:
            ...
            plt.plot(time[window_start:window_start+window_size], ac_s[window_start:window_start+window_size], label='Auto-correlation')
            plt.xlabel('Time (s)')
            plt.ylabel('Amplitude')
            plt.title('Auto-correlation vs Signal')
            plt.legend(loc="upper right")
    ...
    if yin_diff is not None:
        if window_size == -1:
            ...
            plt.figure()
            plt.plot(time[window_start:window_start+window_size], signal[window_start:window_start+window_size], label='Signal')
            plt.plot(time[window_start:window_start+window_size], yin_diff[window_start:window_start+window_size], label='YIN difference function')
            plt.xlabel('Time (s)')
            plt.ylabel('Amplitude')
            plt.title('YIN Difference vs Signal')
            plt.legend(loc="upper right")
